...
//    
    ...
    try {
        ...
        writer.write(cCode);
        writer.close();
        System.out.println("Temporary C file created at: " + tempFile.getAbsolutePath());
    } catch (IOException e) {
        e.printStackTrace();
        return;
    }
    ...
    try {
        ...
        String line;
        while ((line = reader.readLine()) != null) {
            System.out.println("Compile Output: " + line);
        }

        int compileResult = compileProcess.waitFor();
        if (compileResult != 0) {
            System.err.println("Compilation failed with exit code: " + compileResult);
            return;
        } else {
            System.out.println("Compilation successful, output at: " + outputPath);
        }
    } catch (IOException | InterruptedException e) {
        e.printStackTrace();
        return;
    ...
    try {
        System.out.println("Executing the compiled program: " + outputPath);
        ProcessBuilder processBuilder = new ProcessBuilder(outputPath);
        processBuilder.redirectErrorStream(true); // Combine stdout and stderr
        ...
        // Read any errors from the program
        ...
        boolean exeDeleted = new File(outputPath).delete();
        System.out.println("Temporary C file deleted: " + tempDeleted);
        System.out.println("Compiled executable deleted: " + exeDeleted);
    } catch (IOException | InterruptedException e) {
        e.printStackTrace();
    }
   }
